...
const CreateTrainingProgram: React.FC = () => {
  ...
  const onSubmit = async (data: CreateTrainingProgramRequest) => {
    ...
    try {
      await createTrainingProgram(user, data);
      setMessage("Training Program Created Successfully!");
    } catch (error: any) {
      setMessage(
        error.response?.data?.message || "Failed to create training program."
      );
    } finally {
      setLoading(false);
    }
    ...
    <Box p={5} borderWidth={1} borderRadius="lg">
      <VStack as="form" gap={4} onSubmit={handleSubmit(onSubmit)}>
        <Controller
          ...
          defaultValue=""
          render={({ field }) => (
            <Input {...field} placeholder="Program Name" required />
          )}
        />
        <Controller
          name="description"
          ...
          defaultValue=""
          render={({ field }) => (
            <Textarea {...field} placeholder="Program Description" required />
          )}
        />
